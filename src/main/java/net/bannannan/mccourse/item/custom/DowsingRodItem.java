package net.bannannan.mccourse.item.custom;
...
public class DowsingRodItem extends Item {
        ...
        if(context.getWorld().isClient){
            ...
            for(int i = 0; i <= positionClicked.getY(); i++){
                ...
                if(isValuableBlock(blockBelow)) {
                    outPutValuableCoordMessage(positionClicked, player, blockBelow);
                    foundBlock = true;
                    break;
                }
            }
    ...
    private static void outPutValuableCoordMessage(BlockPos blockPos, PlayerEntity player, Block blockBelow) {
        player.sendMessage(new LiteralText("Found " + blockBelow.asItem().
